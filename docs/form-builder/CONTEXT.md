...
- Keep last **N schema versions** on CDN (see [schema rollback runbook](./runbooks/schema-rollback.md)).
- Toggle to pin a specific schema version per environment or manifest `pinnedVersion`.
- One‑liner helper (`scripts/pin-form-schema-version.js`) flips versions; verify in staging after each change.

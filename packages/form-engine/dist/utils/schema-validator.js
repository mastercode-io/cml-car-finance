...
export class SchemaValidator {
    ...
    registerCustomFormats() {
        ...
        const postcodeValidator = (data) => /^[A-Z]{1,2}[0-9]{1,2}[A-Z]?\s?[0-9][A-Z]{2}$/i.test(data.trim());
        ...
        this.ajv.addFormat('iban', {
            type: 'string',
            validate: (data) => /^[A-Z]{2}[0-9]{2}[A-Z0-9]{4}[0-9]{7}([A-Z0-9]?){0,16}$/.test(data),
        });
        this.ajv.addFormat('currency', {
            type: 'string',
            validate: (data) => /^\d+(\.\d{1,2})?$/.test(data),
        });
    }
            ...
            compile(schema) {
                const validator = (data) => {
                    if (!schema || typeof schema !== 'object')
                        return true;
                    const { field, equals } = schema;
                    if (!field)
                        return true;
                    if (data[field] === equals) {
                        const required = Array.isArray(schema.requires) ? schema.requires : [];
    ...
    validateSchema(schema) {
        ...
        const ajvErrors = (this.ajv.errors || []).map((error) => ({

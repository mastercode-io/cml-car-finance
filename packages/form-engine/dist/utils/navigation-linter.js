export const lintNavigationSchema = (schema) => {
    const errors = [];
    const warnings = [];
...
const detectDuplicateSteps = (stepIds, errors) => {
    const seen = new Set();
...
const detectMultipleDefaults = (transitions, errors) => {
...
const detectUnknownReferences = (transitions, stepSet, errors) => {
    transitions.forEach((transition) => {
...
const detectCycles = (transitions, stepSet, errors) => {
...
const detectMissingTerminals = (stepIds, transitions, stepSet, warnings) => {
    if (stepIds.length === 0) {
        return;
    }
...
//# sourceMappingURL=navigation-linter.js.map